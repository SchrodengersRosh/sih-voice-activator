"""M1 WebSocket PCM receiver. Run with: python -m backend.server"""
...
from .protocol import CODEC_PCM, CODEC_NAMES, FLAG_FIRST, PCM_PAYLOAD_LEN, SAMPLES_PER_FRAME, AudioFrame, ProtocolError, unpack
...
class VoiceServer:
    ...
    async def handler(self, ws: ServerConnection) -> None:
        ...
        try:
            while True:
                try:
                    message = await asyncio.wait_for(ws.recv(), timeout=self.inactivity_s if streams else None)
                except TimeoutError:
    ...
    async def _control(self, ws: ServerConnection, raw: str, hello: dict[str, Any] | None, streams: dict[int, Stream]) -> dict[str, Any] | None:
        try:
            msg = json.loads(raw)
        ...
        if kind == "hello":
            if msg.get("proto") != 1 or not isinstance(msg.get("device_id"), str):
                ...
                return hello
            if msg.get("sample_rate") != SAMPLE_RATE or "pcm_s16le" not in msg.get("codecs", []):
        ...
        if kind == "start":
            ...
            if not isinstance(stream_id, int) or not 0 <= stream_id <= 0xFFFFFFFF:
                ...
                return hello
            if (msg.get("codec"), msg.get("sample_rate"), msg.get("channels"), msg.get("frame_ms")) != ("pcm_s16le", 16000, 1, 20):
                await ws.send(json.dumps({"type": "error", "code": "UNSUPPORTED_FORMAT"}))
                return hello
    ...
    def _audio(self, raw: bytes, streams: dict[int, Stream]) -> None:
        ...
        if frame.codec != CODEC_PCM:
            stream.bad += 1
            LOG.warning("dropped non-PCM frame stream=%d", frame.stream_id)
            return
        ...
        if frame.sample_offset != stream.expected_offset:
            ...
            if stream.expected_offset < frame.sample_offset <= stream.expected_offset + MAX_GAP_SAMPLES:
                ...
                stream.bad += 1
                LOG.warning("dropped invalid offset=%d expected=%d stream=%d", frame.sample_offset, stream.expected_offset, frame.stream_id)
                return
    ...
    def finalize(self, stream: Stream, reason: str) -> Path:
        ...
        LOG.info("finalized stream=%d reason=%s frames=%d gaps=%d bad=%d path=%s", stream.stream_id, reason, stream.frames, stream.gaps, stream.bad, path)
        return path


async def run(host: str, port: int, output_dir: Path) -> None:
    app = VoiceServer(output_dir)
    async with serve(app.handler, host, port, max_size=2048, ping_interval=20, ping_timeout=20):
